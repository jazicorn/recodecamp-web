...
const D_Category_Menu = ({menuData}) => {
    /** Custom Hooks */
    ...
    <div className={` ${isMobile ? "tw-pb-4 tw-pt-2" : ""} tw-h-full`}>
        <Transition>
            <div className={`${darkMode ? "" : ""} tw-py-2 tw-h-full tw-mb-2 tw-z-50 tw-relative`}>
                ...
                {menuDropdown &&
                    <div ref={ref} 
                    className={`${darkMode ? "tw-bg-campfire-neutral-800/95" : "tw-bg-campfire-neutral-100/95"} tw-border-no-border tw-rounded 
                    tw-z-300 tw-absolute tw-h-auto tw-w-[14em] tw-p-4 tw-ml-24`}>
                        <ul className={`${darkMode ? "" : ""} tw-inline-block`}>
                            {Object.entries(languages).map(([key, value], index) => {
                                return ( 
                                    <li key={index} className={`${darkMode ? "tw-border-campfire-neutral-600" : "tw-border-campfire-neutral-400"} tw-border-b tw-flex tw-flex-row tw-gap-x-2 tw-pt-3 tw-items-center`}>
                                        <img src={value} alt="key" style={{ height: 20, width: 20 }}/>
                                        <button onClick={() => refetchData(key)} 
        ...
        <ul className={`${darkMode ? "marker:tw-text-campfire-neutral-400" : "" } 
