...
const HomeBanner = () => {
  ...
  return (
    <section
      className={`${
        darkMode
          ? 'tw-backdrop-opacity-70 tw-bg-black/70 [&_h1]:tw-text-campfire-neutral-200'
          : ' tw-backdrop-opacity-70 tw-bg-white/50'
      } ${isMobile ? 'tw-flex-col tw-pb-4' : 'tw-flex-row-reverse tw-min-w-[800px] tw-min-h-[450px]'}
          tw-flex tw-place-items-center tw-place-content-center tw-h-full tw-w-full tw-font-space_mono`}
    >
      {/** Stock Picture */}
      <div
        className={`${
          darkMode ? 'custom-bg-shape-darkest' : 'custom-bg-shape-lighest'
        }  tw-h-full tw-w-full tw-flex tw-flex-col tw-justify-center tw-items-center tw-basis-1/2 pr-20 tw-mr-4`}
      >
      ...
      <div className="tw-basis-1/2 tw-pl-4 tw-w-full tw-flex tw-flex-col tw-place-content-center tw-place-items-center">
        <Transition>
          {isMobile && (
            <div
              className={`${
                darkMode ? 'border-gradient-dark' : 'border-gradient'
              } tw-flex tw-flex-col tw-h-full tw-pb-8`}
            >
              ...
              >
                ...
                <span className="tw-text-campfire-blue">arrays, functions, objects,</span> and other programming
                fundamentals
              </p>
              {/**Link to dashboard */}
              <Link
                to={`/auth/guest/signup`}
                id="banner-dashboard-link"
                className={`${
                  darkMode
                    ? 'hover:tw-text-campfire-neutral-300 tw-text-campfire-purple-light hover:tw-decoration-campfire-blue'
              ...
              >
                <span
                  className={`${
                    darkMode ? 'hover:tw-decoration-campfire-blue' : 'hover:tw-decoration-campfire-neutral-500'
                  } tw-font-space_mono_bold
              ...
              <ul
                className={`${
                  darkMode ? 'tw-text-campfire-neutral-400' : ''
                } tw-flex tw-flex-row tw-place-content-center
                [&>li]:tw-flex [&>li]:tw-flex-row [&>li]:tw-items-center 
                tw-mt-2 tw-gap-4`}
              >
                <li className="">
                  <JS style={{ height: 20, width: 20 }} />
                  &nbsp;Javascript
                </li>
                <li>
                  <Java style={{ height: 20, width: 20 }} />
                  &nbsp;Java
                </li>
                <li>
                  <Python style={{ height: 20, width: 20 }} />
                  &nbsp;Python
                </li>
              </ul>
            </div>
          ...
          {isDesktopMDXL && (
            <div
              className={`${
                darkMode ? 'border-gradient-dark' : 'border-gradient'
              } tw-flex tw-flex-col tw-w-full tw-pl-10 tw-pr-4 `}
            >
              <div className={`${darkMode ? '' : ''} tw-px-2 tw-py-4`}>
                ...
                <ul
                  className={`${
                    darkMode ? 'tw-text-campfire-neutral-400' : ''
                  } tw-flex tw-flex-row tw-place-content-start
                  [&>li]:tw-flex [&>li]:tw-flex-row [&>li]:tw-items-center
                  tw-mt-2 tw-gap-4`}
                >
                  <li className="">
          ...
          {isDesktopXL && (
            <div
              className={`${
                darkMode ? 'border-gradient-dark' : 'border-gradient'
              } tw-flex tw-flex-col tw-w-full tw-pl-20 tw-p-5`}
            >
              ...
              <Link
                ...
                className={`${
                  darkMode
                    ? 'hover:tw-text-campfire-neutral-300 tw-text-campfire-purple hover:tw-decoration-campfire-blue'
                    : 'tw-text-campfire-purple hover:tw-text-campfire-blue '
                } tw-font-space_mono_bold
                    tw-w-42 tw-my-2  tw-py-1 tw-flex tw-flex-row tw-place-self-left`}
              >
              ...
              <ul
                className={`${darkMode ? 'tw-text-campfire-neutral-400' : ''} tw-flex tw-flex-row tw-place-content-start
                [&>li]:tw-flex [&>li]:tw-flex-row [&>li]:tw-items-center
                tw-mt-2 tw-gap-4`}
              >
                <li className="">
