  ...
  return (
    ...
    >
      <div
        className={`${
          darkMode
            ? 'custom-bg-waves-neutral-darker-inverse tw-bg-blend-overlay tw-opacity-60 '
            : 'custom-bg-waves-neutral-lighest tw-bg-blend-overlay tw-opacity-60'
        } tw-w-full tw-h-full  tw-rounded`}
      >
        <section
          className={`${
      ...
      [&>main>ul]:tw-flex [&>main>ul]:tw-flex-col [&>main>ul]:tw-gap-2`}
        >
          <h4
            className={`${
              darkMode
                ? 'tw-text-campfire-neutral-300 hover:tw-text-2 campfire-neutral-300'
                : 'tw-text-campfire-neutral-700 hover:tw-text-campfire-neutral-700'
            } tw-border-campfire-purple-light
          tw-border-b tw-text-2xl tw-h-[40px] tw-w-full tw-pl-3 tw-pb-3 tw-mb-4`}
          >
            <div
              className={`${
                darkMode ? 'tw-border-campfire-neutral-800' : 'tw-border-campfire-blue'
              } tw-border-l-2 tw-px-1.5 tw-flex tw-flex-row tw-place-self-center tw-h-full tw-px-2`}
            >
          ...
          `}
            >
              ...
              <li
                className={`${
                  darkMode
                    ? 'hover:tw-text-campfire-neutral-300'
                    : 'hover:tw-text-campfire-neutral-700 tw-text-campfire-blue '
                }`}
              >
          ...
          `}
            >
              ...
              <li
                className={`${darkMode ? 'hover:tw-text-campfire-neutral-300' : 'hover:tw-text-campfire-neutral-700'}`}
              >
                <Link to={'/learn/docs'}>{darkMode ? <IconBook2 color="#d4d4d4" /> : <IconBook2 color="#000" />}</Link>
              </li>
              <li
                className={`${darkMode ? 'hover:tw-text-campfire-neutral-300' : 'hover:tw-text-campfire-neutral-700'}`}
              >
                <Link to={'/learn/notes'}>{darkMode ? <IconNotes color="#d4d4d4" /> : <IconNotes color="#000" />}</Link>
              </li>
            </ul>
            <hr
              className={`${darkMode ? 'tw-border-campfire-neutral-800' : 'tw-border-campfire-blue'} 
        tw-place-self-left tw-ml-4 tw-mt-5 tw-mb-4 tw-h-[2px] tw-w-[36px]`}
            ...
            <ul
              ...
              } tw-flex tw-flex-col tw-flex-wrap tw-h-full tw-w-fit 
