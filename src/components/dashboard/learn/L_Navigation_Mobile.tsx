// Compoenent: Dashboard Navigation | Mobile
/** React */
import { 
  useContext, 
  useCallback, 
    ...
    <menu className={`${darkMode ? '' : ''} tw-flex tw-flex-col tw-py-2 tw-pl-2 tw-place-content-start tw-h-full `}>
      {!menu ? (
        <button
          ...
          className={`${
            darkMode
              ? 'tw-bg-campfire-neutral-500/70 hover:tw-bg-campfire-neutral-300/70'
              : 'tw-bg-campfire-neutral-300/70 hover:tw-bg-campfire-neutral-400/70'
        ...
        <div className={`${darkMode ? '' : ''} tw-mr-2 tw-z-30 tw-relative tw-rounded`}>
          <button
            ...
            className={`${
              darkMode
                ? 'tw-bg-campfire-neutral-500/70 hover:tw-bg-campfire-neutral-300/70'
                : 'tw-bg-campfire-blue-100/70 hover:tw-bg-campfire-neutral-400/70'
            } tw-my-1  tw-mr-2 tw-border-transparent tw-rounded tw-w-full`}
          >
          ...
          {menu && (
            <div
              ...
              className={`${
                darkMode
                  ? 'tw-bg-campfire-blue-100/70 tw-border-campfire-neutral-600'
                  : 'tw-bg-campfire-neutral-300/70 tw-border-campfire-blue-200'
              } tw-border tw-rounded-b tw-p-2`}
            >
              <ul className="tw-flex tw-flex-col tw-place-items-start tw-space-y-2 [&>li]:tw-h-[30px] tw-py-1">
                <li
                  className={`${
                    darkMode
                      ? 'hover:tw-bg-campfire-neutral-400 hover:tw-text-campfire-neutral-200'
                      : 'hover:tw-bg-campfire-neutral-100 hover:tw-text-campfire-blue'
                  } 
              ...
              tw-flex tw-flex-row tw-gap-1 tw-w-full tw-py-1`}
                >
                  <Link
                    to="/learn/code"
                    className="tw-flex tw-flex-row tw-gap-1 tw-w-full tw-place-items-center tw-h-[18px]"
                  >
                    {darkMode ? <IconTerminal2 color="#2ca9bc" /> : <IconTerminal2 color="#000" />}
                    <span
                      className={`${
                        darkMode
                          ? 'hover:tw-text-campfire-neutral-300 tw-bg-campfire-neutral-400'
        ...
        tw-place-self-left tw-ml-8 tw-my-3 tw-h-[px] tw-w-[100px]`}
              ...
              tw-flex tw-flex-row tw-gap-1 tw-w-full tw-py-1`}
                >
                  <button
                    onClick={(e) => logout(e)}
                    className="tw-font-space_mono tw-flex tw-flex-row tw-place-items-center tw-text-base tw-w-full"
                  >
                    {darkMode ? <IconLogout color="#2ca9bc" /> : <IconLogout color="#000" />}
                    <span className="tw-pl-1">Logout</span>
                  </button>
                </li>
              </ul>
            </div>
...
export default L_Navigation_Mobile;
