...
//import ErrorDashboard from '../../../../learn/L_Error';
import { LoadingDashboardMD } from '../../../learn/L_Loading';
/** Notifications */
...
import { 
  _LANGUAGES_CODE_MIRROR, 
  _LANGUAGES_RAPID_API, 
  _LANGUAGES_SHORTHAND 
} from '../../../../utils/constants/constLanguages';
import { 
  _QUESTION_ROUTE, 
  _CATEGORIES_ROUTE, 
  _LANGUAGES_ROUTE 
...
const L_Editor = () => {
  ...
  const onSubmission = useCallback(
    ...
    [
      editor,
      getMenuQuestion._QUESTION_RESULT,
      getMenuQuestion._QUESTION_POINTS,
      getMenuLanguage,
      consoleTest,
      setPoints,
      getMenuPoints,
  ...
  if (isSuccess)
    return (
      <div className={`${darkMode ? '' : ''} tw-text-campfire-blue tw-flex tw-flex-col tw-h-full tw-p-2`}>
        <div
          className={`${darkMode ? 'tw-bg-campfire-neutral-600/70' : 'tw-bg-campfire-neutral-200/90'} 
      tw-w-full tw-h-full tw-flex tw-flex-col tw-items-between`}
        >
          ...
          <Transition2>
            <div
              className={`${darkMode ? '' : ''} tw-flex tw-flex-row tw-justify-between tw-content-center tw-h-[36px]`}
            >
              <button
                onClick={(e) => onSubmission(e)}
                className={`${
                  darkMode ? 'hover:tw-bg-campfire-neutral-500' : 'hover:tw-bg-campfire-neutral-100'
                } tw-border-campfire-purple-light tw-border-t tw-font-gro tw-w-full tw-h-full hover:tw-text-campfire-purple-light tw-border-b`}
              >
                Submit
              </button>
            </div>
